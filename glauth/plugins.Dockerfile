...
ADD https://raw.githubusercontent.com/glauth/glauth/${GLAUTH_RELEASE}/v2/scripts/docker/start-plugins.sh /app/docker/
ADD https://raw.githubusercontent.com/glauth/glauth/${GLAUTH_RELEASE}/v2/scripts/docker/default-config-plugins.cfg /app/docker/
ADD https://raw.githubusercontent.com/glauth/glauth/${GLAUTH_RELEASE}/v2/scripts/docker/gl.db /app/docker/

COPY --from=prep /app/v2/glauth /app/glauth
COPY --from=prep /app/v2/*.so /app/
